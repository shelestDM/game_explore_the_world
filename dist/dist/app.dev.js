...
var count_of_collected_resources = 0;
var player_size = {
  x: 50,
  y: 50
};
...
document.addEventListener("keydown", function (event) {
  ...
  if (player_coordinate.x < 0 || player_coordinate.x > window_size.x - player_size.x || player_coordinate.y < 0 || player_coordinate.y > window_size.y - player_size.y) {
...
function returnPlayerToMap() {
  ...
  } else if (player_element && player_coordinate.x > window_size.x - player_size.x) {
    player_coordinate.x = window_size.x - player_size.x;
  } else if (player_element && player_coordinate.y > window_size.y - player_size.y) {
    player_coordinate.y = window_size.y - player_size.y;
  }
}

function checkIsResourceCollected() {
  var condition = player_coordinate.x < resource_coordinate.x + resource_size.x && player_coordinate.x + player_size.x > resource_coordinate.x && player_coordinate.y < resource_coordinate.y + resource_size.x && player_coordinate.y + player_size.x > resource_coordinate.y;
